...
cfg.DATA_LIST_SOURCE = str(project_root / "framework/dataset/gta5_list/{}.txt")
cfg.DATA_LIST_TARGET = str(project_root / "framework/dataset/cityscapes_list/{}.txt")
# Directories
...
cfg.TRAIN.SET_SOURCE_VAL = "val"
cfg.TRAIN.SET_TARGET = "train"
cfg.TRAIN.SET_TARGET_VAL = "val"
...
cfg.TRAIN.RECORD_IMAGES_EVERY = 2000
cfg.TRAIN.SRC_TRAIN_IMAGE_INDEX = 10
cfg.TRAIN.TRG_TRAIN_IMAGE_INDEX = 10
cfg.TRAIN.SRC_VAL_IMAGE_INDEX = 2
cfg.TRAIN.TRG_VAL_IMAGE_INDEX = 10
cfg.TRAIN.MEASURE_PREDICTION_TIME = 20000
# validationparams
# cfg.TRAIN.VALIDATION_PERC = 0.05
cfg.TRAIN.BATCH_SIZE_VAL = 1
cfg.TRAIN.VALIDATION_STEP = 600
...
cfg.TEST.INFO_TARGET = str(project_root / "framework/dataset/cityscapes_list/info.json")
    ...
    for k, v in a.items():
        ...
        if old_type is not type(v):
            if isinstance(b[k], np.ndarray):
                ...
                raise ValueError(
                    f"Type mismatch ({type(b[k])} vs. {type(v)}) "
                    f"for config key: {k}"
                )
