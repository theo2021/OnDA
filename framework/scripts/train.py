...
def get_arguments():
    ...
    parser.add_argument(
        "--random-train", action="store_true", help="not fixing random seed."
    )
    parser.add_argument(
        "--tensorboard",
        action="store_true",
    ...
    parser.add_argument(
        "--viz-every-iter", type=int, default=None, help="visualize results."
    )
    parser.add_argument(
        "--exp-suffix", type=str, default=None, help="optional experiment suffix"
    )
...
def main():
    ...
    if cfg.EXP_NAME == "":
        cfg.EXP_NAME = (
            f"{cfg.SOURCE}2{cfg.TARGET}_{cfg.TRAIN.MODEL}_{cfg.TRAIN.DA_METHOD}"
        )
    ...
    print(os.path.expanduser(cfg.TRAIN.RESTORE_FROM))
    assert osp.exists(
        cfg.TRAIN.RESTORE_FROM
    ), f"Missing init model {cfg.TRAIN.RESTORE_FROM}"
    if cfg.TRAIN.MODEL == "DeepLabv2":
        model = get_deeplab_v2(
            num_classes=cfg.NUM_CLASSES, multi_level=cfg.TRAIN.MULTI_LEVEL
        )
